...
export function loadAmap(): Promise<AMapNS> {
  ...
  if (!loadPromise) {
    ...
    loadPromise = new Promise((resolve, reject) => {
      ...
      script.src = `https://webapi.amap.com/maps?v=2.0&key=${encodeURIComponent(key)}&plugin=AMap.Driving,AMap.Walking,AMap.Riding,AMap.Transfer,AMap.Geocoder,AMap.PlaceSearch,AMap.Geolocation,AMap.IndoorMap`
      script.async = true
      script.onload = () => {
