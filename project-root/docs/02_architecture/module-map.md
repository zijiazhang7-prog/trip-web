...
- 降低重复 SQL 和重复查询逻辑
- 为排序、筛选、联动展示提供统一基础
- 后续接入 `IndexEngine` 后，优先使用索引命中候选数据，索引不可用或未命中时回退到 Mapper 查询
...
- 目标上作为地图公共能力门面，纯图结构和 Dijkstra 等算法细节逐步下沉到 `GraphEngine`
...
- 日记正文检索
- 可能扩展到更丰富的文本搜索场景
- 当前由 `IndexEngine` 负责 Hash / Trie / 日记正文倒排索引等底层检索结构，`SearchService` 负责搜索调度
- 索引不可用或未构建时，由 Mapper `LIKE` 兜底；已构建索引明确未命中时直接返回空结果
...
- 目标职责：邻接表、有向边、Dijkstra、单源最短路、路径回溯、距离 / 时间边权策略。
- 不负责：数据库查询、Controller 入参、VO 组装、路线历史保存。
- 当前状态：已完成基础抽取，`MapServiceImpl` 负责地图数据加载、策略选择和业务结果转换，`GraphEngine` 负责纯图结构与最短路算法。

## 7.4 IndexEngine
作为查找、检索相关的数据结构支撑，服务于查询和检索能力。

- 目标职责：Hash 精确查找、Trie 前缀匹配、倒排索引。
- 不负责：直接访问 MySQL、处理鉴权、组装业务返回对象。
- 当前状态：第一阶段名称 / 标题 Hash 与 Trie 已落地；第二阶段日记正文字符位置倒排索引已落地。结构化条件仍由 Mapper 处理，索引不可用时使用 `LIKE` 兜底。

## 7.5 CompressionEngine
作为日记正文的无损压缩基础能力。

- 当前职责：基于 Unicode 码点、优先队列和 Huffman 树实现日记正文无损压缩和还原。
- 存储原则：复用 `diary.content_compressed` 保存自描述压缩包，保留 `diary.content_text` 原文不变。
- 当前状态：第一阶段已落地，日记发布时写入压缩结果；压缩失败不阻塞原文保存。
...
当前文档中出现的 `GraphEngine`、`IndexEngine`、`CompressionEngine` 均已作为独立基础支撑能力落地。

当前代码事实应按以下口径理解：

| 能力 | 当前代码状态 | 目标状态 |
| --- | --- | --- |
| 图算法 | 已抽取到 `GraphEngine`，`MapServiceImpl` 保留业务门面 | 后续可扩展 A* 等策略 |
| 检索索引 | 已实现名称 / 标题 Hash、Trie 和日记正文倒排索引 | 后续可扩展分词、权重和增量维护 |
| Top-K | `RankServiceImpl` 已有优先队列 Top-K 能力 | 扩大到推荐、美食、日记、搜索候选排序复用 |
| 日记压缩 | 已实现 `CompressionEngine` 和发布写入 | 后续可补历史数据回填和独立压缩统计 |

上述调整必须保持现有接口契约稳定，优先作为公共能力层内部重构推进。
