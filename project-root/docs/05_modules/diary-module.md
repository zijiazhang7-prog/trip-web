...
### 字段语义
- `diary.heat_score` 保存成功查看详情的累计浏览次数。
- 日记浏览量即日记热度，不混入评分、评论或点赞权重。
- `DiaryVO.heatScore` 使用非负整数返回。

### 详情计数流程
1. 查询日记并校验存在、启用状态；
2. 公开日记允许访问，私有日记仅作者允许访问；
3. 权限校验通过后，由 `DiaryMapper` 执行 `heat_score = heat_score + 1` 原子更新；
4. 重新读取日记并返回更新后的浏览量。

不存在、禁用或无权查看的请求不会增加热度。当前 MVP 每次成功查看均计数，不实现 Redis、浏览日志或时间窗口防刷。

### 复杂度与适用范围
- 数据结构：MySQL 主键索引与单行计数字段。
- 原子更新按主键定位，时间复杂度约为 `O(log n)`，额外空间复杂度为 `O(1)`。
- 适用于课程设计和单体 MySQL 部署；后续如需防刷，可增加浏览日志或缓存去重，但不改变当前热度语义。
...
- 评分聚合与浏览热度分别维护，评分不会直接改写 `heat_score`
