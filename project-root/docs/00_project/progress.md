...
## 3. 当前项目总体状态
- 2026-06-12：完成 Route 路线历史查询基础版及实库 HTTP 回归。实现当前用户分页列表和详情接口，详情直接恢复保存的节点、边和多目标访问顺序快照；越权记录按不存在处理，查询不重新运行 GraphEngine。后端全量 256 个测试通过；停服后已完成 `route_history` 增量迁移，并验证列表分页、详情快照、用户隔离、多目标顺序落库和 GET 查询不新增历史，临时数据已清理。
- 2026-06-12：完成 AIGC 日记照片动画后端 MVP及真实多模态 Provider 扩展。默认保留 `mock-template`，新增可配置的 `openai-compatible` Provider、本地图片 Base64 安全读取、严格 JSON 解析、超时与非法响应降级；AI 调用已移出数据库长事务。后端全量 250 个测试通过，真实厂商带密钥联调、实库动画表回归与前端播放器待验证。
...
- 已实现 `walk/bike/cart` 道路权限过滤，`transportType` 会实际影响合法边和路径结果。
- 当前未执行前端路线规划页面联调。
...
- 该阶段记录已被后续实现覆盖：当前已支持最短时间与交通工具边过滤。
- 当前未执行前端多目标路线页面联调。
...
- 该阶段记录已被后续实现覆盖：当前多目标已支持最短时间和交通工具约束。

## 6. 2026-06-11 路线交通工具策略

- 新增 `RouteTransportType` 与 `EdgeTransportAccess`，统一请求模式和道路通行权限。
- `map_edge.transport_type` 支持 `walk/bike/cart/walk_bike/walk_cart/bike_cart/all`。
- GraphEngine 使用 `(nodeId, actualTransportType)` 状态支持单一交通工具过滤和 mixed 最短时间。
- 时间成本统一为 `distance / (min(工具默认速度, 道路理想速度) * crowdFactor)`。
- 单目标、多目标、返回起点段和路线历史使用一致的交通约束。
- `pathEdges[].transportType` 可还原每条边实际使用的交通工具。
- 全量 `mvn -q test` 通过。
- 当前阻塞：实库 6970 条道路仍均为 `walk`，需补充 bike/cart/组合权限演示子图后才能完成 HTTP 演示验收。
- 当前未做前端多目标路线页面联调。
...
## 8.1 2026-06-12 日记浏览量即热度

- 已将日记 `heat_score` 的业务语义固定为成功查看详情的累计浏览次数。
- 已实现详情权限校验后的数据库原子 +1，并返回更新后的 `heatScore`。
- 已确认日记列表、标题检索和正文检索的 `sortBy=heat` 均直接读取数据库实时值。
- 已新增 `BIGINT UNSIGNED` 迁移脚本、单元测试和 MySQL 并发/排序集成测试。
- 已在 MySQL 8 实库执行 `migrate-diary-view-heat-schema.sql`，确认字段为 `BIGINT UNSIGNED NOT NULL DEFAULT 0`，空值或负值记录为 0。
...
- 已完成 MySQL 8 真实 JWT HTTP 回归：两个用户使用不同偏好时，
  `interest` 首位分别命中对应日记；`heat/rating` 的 Top-4 顺序不受用户身份影响。
- 已确认六次响应契约一致，推荐查询不增加 `heat_score`，临时测试数据清理后剩余为 0。
- 已新增可重复执行的 `project-root/scripts/verify-diary-recommend.ps1` 实库回归脚本。
- 已修正演示初始化 SQL 中越界的热门、拥挤偏好等级，三个演示用户分别使用
  `3/2`、`5/4`、`1/1` 的合法等级组合。
- 已增加 SQL 资源回归测试并验证通过。
- 实库中校园目的地尚无关联美食，完整演示 SQL 的连续两次幂等导入需在美食基础数据补齐后执行。

## 8.4 2026-06-12 目的地与美食推荐真分页

- 已修复目的地推荐固定第 1 页和 100 条候选池限制，支持对完整候选排序后按
  `pageNum/pageSize` 分页。
- 已为美食推荐补充 `pageNum/pageSize`，未传 `topK` 时不再默认截为 10 条。
- 两个接口均保留显式 `topK` 兼容模式，且不修改 Controller 路径和响应包装。
- 已同步 `api-spec.md`、`swagger-draft.yaml`、模块文档和字段冻结清单。
- MySQL 8 HTTP 回归确认目的地第 1、2 页各 32 条且 ID 无重复，
  `total=1315`、`pages=42`。
- 美食实库验证确认 `pageSize=32` 可返回目的地下全部 25 条数据，显式
  `topK=5` 仍返回 5 条。
