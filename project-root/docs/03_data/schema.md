...
### 当前实现边界
1. 已实现三类一级评论的发布、分页列表和软删除/隐藏 API；
2. 第一阶段只展示 `parent_comment_id IS NULL` 的一级评论，回复接口尚未实现；
3. 评论数据暂不参与 `heat_score`、`rating_score` 或评论数聚合；
4. 父评论通过同表自关联保留扩展能力，当前发布接口固定写入空父评论。
